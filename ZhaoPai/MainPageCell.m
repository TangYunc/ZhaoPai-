...
//    _dingImageView.backgroundColor = [UIColor purpleColor];
...
//    _mapLocationImageView.backgroundColor = [UIColor purpleColor];
...
- (void)layoutSubviews{
    ...
    //01.标题
    _titlelabel.frame = CGRectMake(5, 10, 220, 30);
    _titlelabel.text = @"［新手必打］拍拍赚新手任务";
    //02.任务积分
    _integralLabel.frame = CGRectMake(self.width-5-40, _titlelabel.top, 40, _titlelabel.height);
    _integralLabel.text =@"1.0";
    //03.顶视图
    _dingImageView.frame = CGRectMake(_integralLabel.left-30-40, _integralLabel.top, 40, 40);
    _dingImageView.image = [UIImage imageNamed:@""];
    //04.隐藏的东西
    _hiddenLabel.frame = CGRectMake(_titlelabel.left, _titlelabel.bottom, _titlelabel.width, 20);
    _hiddenLabel.text = @"(160)";
    //05.需要定位的图像
    _mapLocationImageView.frame = CGRectMake(_dingImageView.left, _dingImageView.bottom, 20, 20);
    _mapLocationImageView.image = [UIImage imageNamed:@""];
    //06.关注
    _attentionLabel.frame = CGRectMake(_titlelabel.left, _hiddenLabel.bottom, 120, 20);
